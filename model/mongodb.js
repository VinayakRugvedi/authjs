...
async function insert(data, isPhone) {
  ...
  let insertResult = await info.save().catch((error) => {
    throw error;
  });
...
async function fetch(key, value) {
  let fetchResult = await AuthAccount.find({ [key]: value }) // Computed Property
    .catch((error) => {
      throw error;
    });
  ...
  return fetchResult.length ? fetchResult[0] : {};
}

async function updateVerified(id) {
  let updateResult = await AuthAccount.findOneAndUpdate(
    { _id: id },
    { $set: { verified: true } }
  ).catch((error) => {
    throw error;
...
async function deleteData(id) {
  let deleteResult = await AuthAccount.findOneAndDelete({ _id: id }).catch(
    (error) => {
      throw error;
    }
  );
