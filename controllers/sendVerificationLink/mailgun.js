...
async function sendVerificationLink(email, token) {
  if (
    authConfig.mailConfiguration.domain === undefined ||
  ...
  if (
    authConfig.mailConfiguration.from !== undefined &&
  ...
  let route;
  if (
    authConfig.mailConfiguration.route === undefined ||
  ...
  if (
    authConfig.mailConfiguration.organizationName !== undefined &&
  ...
  const responseData = await sendMail().catch((error) => {
    throw error;
  });
